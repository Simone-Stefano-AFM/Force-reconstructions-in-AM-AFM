...
# data from "Holscher_Payam_FRM.py file"
I_Holscher = np.loadtxt("k_Holscher3nm.txt")[1, :]
Dmin = np.loadtxt("k_Holscher3nm.txt")[0, :]
amplitude = np.loadtxt("Amp3nm.txt")[1, :]
dforce = np.loadtxt("force.txt")[1, :]
D1 = np.loadtxt("force.txt")[0, :]


# interpolation of data
n = 7000 # higher is better

Dmin_interp = np.linspace(np.min(Dmin), np.max(Dmin), n)

interp_I = si.interp1d(Dmin, I_Holscher, kind = "linear")
I_interp = interp_I(Dmin_interp)
...
force_matrix = inv_mat.dot(-np.pi*np.sqrt(A_interp/2)*I_interp) 
